package FrameworkApp.Sample.Report;
...
public class ReportUtil {
	...
	public static void startTesting(String filename, String testStartTime, String env, String rel) {
		...
		try {
			...
			String RUN_DATE = Common.now("dd.MMMMM.yyyy").toString();
			...
			if (status.equalsIgnoreCase("Skipped") || status.equalsIgnoreCase("Skip")) {
				...
				if (description != null) {
					for (int i = 0; i < description.size(); i++) {
						...
						out.write("<td align=center width=10%><FONT COLOR=#153E7E FACE=Arial SIZE=1><b>" + ExpectedResult.get(i) + "</b></td>");
